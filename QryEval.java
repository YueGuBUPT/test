...
public class QryEval {
  ...
  final static String dummyOutput = " Q0 dummy 1 0 run-1";
  ...
  public static void main(String[] args) throws Exception {
    ...
    // indexPath required for this program to run
    if (!params.containsKey("indexPath")) {
      System.err.println("Error: You must specify a index path to evaluate.");
      System.exit(1);
    }
    
    // queryFilePath required for this program to run
    if (!params.containsKey("queryFilePath")) {
    	System.err.println("Error: You must specify a query file path.");
    	System.exit(1);
    }
    
    // trecEvalOutputPath required for this program to run
    if (!params.containsKey("trecEvalOutputPath")) {
    	System.err.println("Error: You must specify an output file path.");
    	System.exit(1);
    }
    
    // retrievalAlgorithm required for this program to run
    if (!params.containsKey("retrievalAlgorithm")) {
    	System.err.println("Error: You must specify a retrieval algorithm.");
    	System.exit(1);
    }
    ...
    String outputFilePath = params.get("trecEvalOutputPath");
    ...
    if (params.get("retrievalAlgorithm").equalsIgnoreCase("UnrankedBoolean") ) 
    	...
    	System.err.println("Error: The retrieval model has not been supported yet.\n"
    			+ "Currently supported model: UnrankedBoolean, RankedBoolean");
    	System.exit(1);
    }
     ...
     */
    BufferedReader br = new BufferedReader(new FileReader(params.get("queryFilePath")));
    ...
    while ((queryLine = br.readLine()) != null) {
       ...
       // process the line.
        ...
        String qID = queryLine.split(":")[0];
        QryopSlScore scoreOp = new QryopSlScore(qTree);
        QryResult result = scoreOp.evaluateBoolean(model);
        
        Collections.sort(result.docScores.scores);
        
        try {
        	// Output the first 100 results that is related
        	int size = Math.min(result.docScores.scores.size(), 100);
            for (int i = 0; i < size; i++) {
	            StringBuilder sb = new StringBuilder("");
	            String docID = getExternalDocid(result.docScores.getDocid(i));
	            int score = (int)result.docScores.getDocidScore(i);
	            sb.append(qID + " Q0 " + docID + " " + (i + 1) + " " + score + " run-1\n");
	            String resultString = new String(sb);
	
	            writer.write(resultString);
            }
            if (result.docScores.scores.size() == 0) {
            	writer.write(qID + dummyOutput);
            }
        ...
        //printResults (queryLine, qTree.evaluate (model));
    }
    br.close();
    ...
    while (tokens.hasMoreTokens()) {
      ...
      if (token.matches("[ ,(\t\n\r]")) {
        ...
        stack.push(currentOp);
          ...
          stack.push(currentOp);
      } else if (token.equalsIgnoreCase("#near")) {  // delimit near and n with /, try to find n after near
    	  int n = 0;
        ...
        // the token specifies a particular field (e.g., apple.title).
    	    ...
    	    // Deal with field problem
    	    if (tokenSplit.length == 2) {
    	    	String[] tokenized = tokenizeQuery(tokenSplit[0]);
    	    	if (tokenized.length > 0)
    	    		currentOp.add(new QryopIlTerm(tokenized[0], tokenSplit[1]));
    	    } else if (tokenSplit.length == 1){
    	    	String[] tokenized = tokenizeQuery(tokenSplit[0]);
    	    	if (tokenized.length > 0)
    	    		currentOp.add (new QryopIlTerm(tokenizeQuery(tokenSplit[0])[0]));
    	    }
