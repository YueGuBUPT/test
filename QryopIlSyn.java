...
public class QryopIlSyn extends QryopIl {
  ...
  public QryResult evaluate(RetrievalModel r) throws IOException {
    ...
    //TODO: How to check whether the daatPtrs is not null?
    ...
    while (this.daatPtrs.size() > 0) {
      ...
      // Update the nextDoc ptr of each matched doc
	...
	if (ptri.invList.getDocid (ptri.nextDoc) == nextDocid) {
      ...
      // Merge multiple inverted list results into a new inverted List
  ...
  public QryResult ConvertToScoreList() {
	  ...
	  if (qResult.docScores != null)
		  return qResult;
	  
	  if (qResult.invertedList == null)	return null;
	  
	  for (int i = 0; i < qResult.invertedList.postings.size(); i++) {
		  int docid = qResult.invertedList.postings.get(i).docid;
		  int cnt = qResult.invertedList.postings.get(i).positions.size();
		  
		  qResult.docScores.add(docid, 1.0 * cnt);
	  }
